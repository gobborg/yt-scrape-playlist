// Excepting main() and const(), all of this came from
// https://github.com/youtube/api-samples/blob/master/go/oauth2.go
// But I fixed it to all be :8080 because this was clearly not proofed.
package main
...
const (
	apiKeysFile     = "./client_secret.json"
	credentialsDir  = "/.credentials"
	bearerTokenFile = "youtube-go.json"
...
func OAuth() (*youtube.Service, error) {
	ctx := context.Background()
	homedir, err := user.Current()
	//CREATE A BEARER TOKEN
	keys, err := ioutil.ReadFile(apiKeysFile)
	if err != nil {
		log.Fatalf("Unable to read client_secret file: %v", err)
	}
	config, err := google.ConfigFromJSON(keys, scope)
	if err != nil {
		log.Fatalf("Unable to make a happy config using API keys: %v", err)
	}
	tokenFile := homedir.HomeDir + credentialsDir + "/" + bearerTokenFile
	token, err := tokenFromFile(tokenFile)
	if err != nil {
		fmt.Println("Error:", err)
		errorMessage := err.Error()
		errorPattern := `(?i)no such file or directory`
		regexPattern := regexp.MustCompile(errorPattern)
		switch {
		case regexPattern.MatchString(errorMessage):

			log.Println("Failed to use token from file. Trying to create a token.")
			getClient(scope)
			startWebServer()
		...
		default:
			if err != nil {
				log.Fatalf("Unable to use a token file due to: %v", err)
			}
		}
	}
	tokenBorg := config.TokenSource(ctx, token) // this is some arg for ytClientlient that I don't understand

	// CONNECT TO YOUTUBE
	fmt.Println("Attempting to create ytClient.\n")
	ytClient, err := youtube.NewService(ctx, option.WithTokenSource(tokenBorg))
	if err != nil {
		log.Fatalf("Error creating YouTube client: %v", err)
	}
	if err == nil {
		fmt.Println("ytClient creation success.\n")
	}
	return ytClient, nil
}
